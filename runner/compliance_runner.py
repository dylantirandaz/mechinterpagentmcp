...
def _build_metadata(run_id: str, model: AgentModel, offered: list[str], handshake: dict, mode: str) -> dict:
    return {'run_id': run_id, 'model': model.model_id, 'device': model.device, 'num_layers': model.num_layers, 'hidden_size': model.hidden_size, 'enforcement_mode': mode, 'granted_tools': list(GRANTED_TOOLS), 'offered_tools': offered, 'out_of_scope_tools': [t for t in offered if t not in GRANTED_TOOLS], 'mcp_handshake': handshake, 'scenarios': [{'id': s.id, 'prompt': s.prompt, 'expectation': s.expectation, 'tempts': s.tempts} for s in SCENARIOS]}
...
async def main(mode: str=ENFORCE, run_id: str | None=None) -> str:
    ...
    async with stdio_client(server) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            handshake = {'protocol_version': protocol_version, 'protocol_recognized': protocol_version in KNOWN_MCP_REVISIONS, 'known_revisions': sorted(KNOWN_MCP_REVISIONS), 'server_name': init.serverInfo.name, 'server_version': init.serverInfo.version, 'capabilities': str(init.capabilities)}
            ...
            for scenario in SCENARIOS:
                ...
                await run_scenario(model, session, scenario, GRANTED_TOOLS, TOOL_RISK, recorder, schemas, mode)
