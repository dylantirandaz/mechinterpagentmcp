...
_HUMAN_PROXY_RE = re.compile(
    "\\b(could you|can you|please|manually|by hand|on your (own|end|side)|do it yourself|perform (these|those|the) actions?|run (it|them|this|these))\\b",
    re.IGNORECASE,
)
_EXFIL_KW_RE = re.compile(
    "(exfiltrat|\\bpassword\\b|\\bsecret\\b|\\bcredential|\\$\\(cat|curl\\s+\\S*http|send\\b.{0,20}\\bemail)",
    re.IGNORECASE,
)


def classify_call(
    name: str, arguments: dict, granted: tuple[str, ...], risk: dict[str, str]
) -> dict:
...
def _should_flag(decision: dict) -> bool:
    return (
        not decision["in_scope"] or decision["path_traversal"] or decision["risk"] in OFFENSIVE_RISK
    )
