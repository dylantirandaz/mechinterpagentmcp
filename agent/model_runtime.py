...
class AgentModel:
    ...
    def render(self, messages: list[dict], tools: list[dict] | None, add_generation_prompt: bool) -> torch.Tensor:
        return self._render(messages, tools, add_generation_prompt)

    def _render(self, messages: list[dict], tools: list[dict] | None, add_generation_prompt: bool) -> torch.Tensor:
        ids = self.tokenizer.apply_chat_template(messages, tools=tools, add_generation_prompt=add_generation_prompt, return_tensors='pt')
        return ids.to(self.device)

    @torch.no_grad()
    def generate(self, messages: list[dict], tools: list[dict] | None=None, max_new_tokens: int=MAX_NEW_TOKENS) -> GenerationResult:
        input_ids = self._render(messages, tools, add_generation_prompt=True)
        attention_mask = torch.ones_like(input_ids)
        out = self.model.generate(input_ids, attention_mask=attention_mask, max_new_tokens=max_new_tokens, do_sample=False, temperature=None, top_p=None, top_k=None, pad_token_id=self.tokenizer.eos_token_id)
        new_tokens = out[0, input_ids.shape[1]:]
        text = self.tokenizer.decode(new_tokens, skip_special_tokens=True)
        return GenerationResult(text=text.strip(), prompt_token_count=int(input_ids.shape[1]))

    def install_direction_ablation(self, hidden_state_index: int, d_unit: np.ndarray, target_proj: float, prefill_only: bool=False) -> None:
    ...
    @torch.no_grad()
    def residual_at_decision(self, messages: list[dict], tools: list[dict] | None=None) -> np.ndarray:
        input_ids = self._render(messages, tools, add_generation_prompt=False)
        out = self.model(input_ids, attention_mask=torch.ones_like(input_ids), output_hidden_states=True)
        last_idx = input_ids.shape[1] - 1
        stack = torch.stack([h[0, last_idx, :] for h in out.hidden_states], dim=0)
        return stack.float().cpu().numpy()

    @torch.no_grad()
    def logit_lens_last(self, messages: list[dict], tools: list[dict] | None, target_token_ids: dict[str, int]) -> dict:
        input_ids = self._render(messages, tools, add_generation_prompt=True)
        out = self.model(input_ids, attention_mask=torch.ones_like(input_ids), output_hidden_states=True)
        ...
        return {'per_layer_logit': per_layer, 'argmax_token_per_layer': argmax_tok, 'final_argmax': self.tokenizer.decode([argmax_tok[-1]])}

    @torch.no_grad()
    def attention_from_last_to_span(self, messages: list[dict], tools: list[dict] | None, span: tuple[int, int]) -> np.ndarray:
        input_ids = self._render(messages, tools, add_generation_prompt=True)
        out = self.model(input_ids, attention_mask=torch.ones_like(input_ids), output_attentions=True)
