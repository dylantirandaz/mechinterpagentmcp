...
from probes.common import PROBE_NPZ, emits, load_contrast_rows, offensive_tool_names
...
async def _collect_contexts(model, session, tools) -> list[dict]:
    ...
    for scenario in SCENARIOS:
        ...
        for turn in range(1, MAX_TURNS + 1):
            ...
            offensive = offensive_tool_names(baseline)
            if offensive:
                contexts.append(
                    {
                        ...
                        "offensive": sorted(set(offensive)),
                    }
                )
            ...
            for call in calls:
                ...
                result = (
                    await call_tool(session, call.name, call.arguments)
                    if d["in_scope"]
                    else f"DENIED by runner: '{call.name}' is not in your granted scope."
                )
...
def _removal_rate(model, contexts, tools, layer, d, target, prefill_only=False) -> float:
    ...
    for ctx in contexts:
        ...
        if not any((emits(text, tool) for tool in ctx["offensive"])):
            removed += 1
...
def _sweep_layer(model, contexts, tools, layer, probe, acts, mask_off, mask_ben) -> dict:
    ...
    directions = {
        "diffmean": diff_of_means_direction(mu_off, mu_ben),
        "probe": raw_unit_direction(probe[f"coef_{layer}"], probe[f"scale_{layer}"]),
        "random0": random_unit_direction(acts.shape[2], RANDOM_SEEDS[0]),
        "random1": random_unit_direction(acts.shape[2], RANDOM_SEEDS[1]),
    }
    rec = {"layer": layer}
    for name, d in directions.items():
        rec[name] = _removal_rate(model, contexts, tools, layer, d, float((benign @ d).mean()))
    dd = directions["diffmean"]
    rec["diffmean_prefill"] = _removal_rate(
        model, contexts, tools, layer, dd, float((benign @ dd).mean()), prefill_only=True
    )
...
def _summarize(results: list[dict], n_contexts: int) -> dict:
    ...
    specific = [
        ...
        for r in results
        for dirn in ("diffmean", "probe", "diffmean_prefill")
        if r[dirn] >= SPECIFIC_MIN_REMOVAL and r[dirn] - r["random_mean"] >= SPECIFIC_MIN_MARGIN
    ]
    return {
        "n_offensive_decisions": n_contexts,
        "layers_swept": len(results),
        "best_diffmean": best("diffmean"),
        "best_probe": best("probe"),
        "best_diffmean_prefill": best("diffmean_prefill"),
        "specific_causal_hits": specific,
        "causal_reading": f"causal locus found: {len(specific)} (layer,direction) hit(s) beat the random control by >= {SPECIFIC_MIN_MARGIN}"
        if specific
        else "ROBUST NULL: no layer/direction/position specifically suppressed the offensive emission beyond the random control",
        "evidence_type": "causal (layer x direction x position ablation sweep with random controls)",
    }
...
async def main(run_id: str) -> Path:
    ...
    async with stdio_client(server) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            print(
                "[sweep] collecting offensive decision contexts (baseline trajectory)...",
                flush=True,
            )
            contexts = await _collect_contexts(model, session, tools)
            print(
                f"[sweep] {len(contexts)} offensive decisions; sweeping {acts.shape[1] - 1} layers...",
                flush=True,
            )
            results = []
            for layer in range(1, acts.shape[1]):
                ...
                results.append(rec)
                print(
                    f"  L{layer:2d} diffmean {rec['diffmean']} probe {rec['probe']} prefill {rec['diffmean_prefill']} random {rec['random_mean']}",
                    flush=True,
                )
        ...
        json.dumps(
            {
                "summary": summary,
                "decisions": [
                    {"scenario": c["scenario"], "turn": c["turn"], "offensive": c["offensive"]}
                    for c in contexts
                ],
                "per_layer": results,
            },
